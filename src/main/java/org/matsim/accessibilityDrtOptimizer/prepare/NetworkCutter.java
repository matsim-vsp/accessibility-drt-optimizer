...
public class NetworkCutter implements MATSimAppCommand {
    ...
    public Integer call() throws Exception {
        ...
        for (Link link : network.getLinks().values()) {
            ...
            } else if (!from.within(areaToKeep) || !to.within(areaToKeep)) {
                linksToRemove.add(link);
            }
        }
        ...
        for (Link link : network.getLinks().values()) {
            if (!link.getAllowedModes().contains(TransportMode.pt)){
                link.setAllowedModes(Set.of(TransportMode.car, TransportMode.drt));
            }
        }

        if (keepPT) {
            MultimodalNetworkCleaner networkCleaner = new MultimodalNetworkCleaner(network);
            networkCleaner.run(Set.of(TransportMode.car, TransportMode.drt));
        } else {
